...
public class ParseService {
    ...
    @Value("${file.parsing.overlap-size:200}")
    ...
    /**
     * 对相邻文本块应用简单滑动窗口重叠：将前一块的结尾片段拼接到后一块的开头，
     * 在不显著放大存储体量的前提下保证跨块语义的连续性。
     */
    private List<String> applyOverlap(List<String> originalChunks) {
        if (originalChunks == null || originalChunks.size() <= 1 || overlapSize <= 0) {
            return originalChunks;
        }

        List<String> overlapped = new ArrayList<>(originalChunks.size());
        // 第一块保持不变
        overlapped.add(originalChunks.get(0));

        for (int i = 1; i < originalChunks.size(); i++) {
            String prev = originalChunks.get(i - 1);
            String current = originalChunks.get(i);
            ...
            String tail;
            if (prev.length() <= overlapSize) {
                tail = prev;
            } else {
                tail = prev.substring(prev.length() - overlapSize);
            }

            StringBuilder builder = new StringBuilder(tail.length() + 2 + current.length());
            builder.append(tail);
            if (!tail.endsWith("\n\n")) {
                builder.append("\n\n");
            }
